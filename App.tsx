...
const App: React.FC = () => {
  ...
  const renderDescription = (text: string) => {
    ...
    return parts.map((part, i) => {
      ...
      if (part.match(/^https?:\/\//)) {
        const isMap = part.includes('google.com/maps') || part.includes('maps.google') || part.includes('goo.gl/maps');
        return (
  ...
  const renderSectionContent = (section: any) => {
    ...
    if (section.id === 'checkin') {
      ...
      const checklistKeys = ['lights', 'belongings', 'windows', 'keys', 'host'];
        ...
        <div className="px-6 pb-8 pt-2 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-emerald-950/5 border border-emerald-900/10 p-5 rounded-[1.5rem] flex flex-col items-center justify-center text-center relative overflow-hidden">
              ...
              <div className="text-3xl font-black text-emerald-600 tracking-tight">13:30</div>
            </div>
            <div className="bg-rose-950/5 border border-rose-900/10 p-5 rounded-[1.5rem] flex flex-col items-center justify-center text-center relative overflow-hidden">
              ...
              <div className="text-3xl font-black text-rose-600 tracking-tight">10:30</div>
            </div>
          </div>
          ...
          <div className="p-5 bg-stone-50 border border-stone-100 rounded-[1.8rem]">
            ...
            <div className="space-y-3">
              {locations.map((loc: string, idx: number) => {
                ...
                return (
                  <a key={idx} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 rounded-xl bg-white border border-stone-200 hover:border-amber-200 hover:bg-amber-50 transition-all group">
          ...
          {items.map((item: string, idx: number) => {
            ...
            return (
              <div key={idx} className="bg-white rounded-[1.8rem] border border-stone-200 p-5 shadow-sm hover:border-amber-200 transition-all group">
                ...
                {
                  mapsUrl && (
                    <a href={mapsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 rounded-xl bg-stone-50 border border-stone-100 hover:bg-stone-100 transition-colors group/link">
  ...
  return (
    <div className="min-h-screen pb-10 max-w-2xl mx-auto bg-stone-50 text-stone-800 selection:bg-amber-100 font-sans">
      ...
      <div className="relative h-[24rem] overflow-hidden border-b border-stone-200 shadow-sm">
        <img src="https://lh3.googleusercontent.com/d/1XMsSNiMg-9Fllm2v4j7suOR7gEvjOCD2" alt="Nitto House Bedroom" className="w-full h-full object-cover scale-105" />
        <div className="absolute inset-0 bg-gradient-to-t from-stone-50 via-stone-50/20 to-transparent" />

        <div className="absolute top-6 right-6 flex gap-2">
          {Object.values(Language).map(l => (
            <button key={l} onClick={() => setLang(l)}
              className={`w-10 h-10 rounded-full flex items-center justify-center overflow-hidden border-2 transition-all ${lang === l
        ...
        <div className="absolute bottom-10 left-8 right-8">
          <div className="flex items-center gap-2 mb-3">
            <span className="px-3 py-1 bg-amber-100 text-amber-800 border border-amber-200 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">Guest House</span>
          </div>

          <div className="mb-2">
            <h1 className="text-4xl sm:text-5xl font-serif font-bold text-blue-500 drop-shadow-sm tracking-widest uppercase">
              {data.propertyName}
            </h1>
          </div>
      ...
      <div className="px-6 py-10 space-y-10">
        ...
        <div className="grid grid-cols-2 gap-5">
          <a href={`https://www.google.com/maps/dir/?api=1&amp;destination=${encodeURIComponent(data.address)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center p-6 bg-white rounded-[2rem] border border-stone-200 shadow-sm transition-all hover:shadow-md hover:border-amber-200 active:scale-95 group">
          ...
          <a href={`https://wa.me/${cleanPhone(data.contacts.whatsapp)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center p-6 bg-white rounded-[2rem] border border-stone-200 shadow-sm transition-all hover:shadow-md hover:border-emerald-200 active:scale-95 group">
        ...
        <div className="space-y-5">
          {data.sections.map((section) => {
            ...
            return (
              <div key={section.id} className={`bg-white rounded-[2.5rem] border overflow-hidden transition-all duration-300 ${isExpanded ? 'border-amber-200 shadow-md ring-1 ring-amber-50' : 'border-stone-200 shadow-sm hover:border-stone-300'}`}>
                {/* Header */}
                <button onClick={() => toggleSection(section.id)}
                  className="w-full text-left p-6 flex items-center justify-between group"
                >
                  <div className="flex items-center gap-5">
                    <div className={`p-4 rounded-2xl border transition-all ${isExpanded ? 'bg-amber-800 border-amber-800 text-white' : 'bg-stone-50 border-stone-100 text-stone-500 group-hover:border-amber-200 group-hover:text-amber-800'}`}>
        ...
        <div className="bg-white rounded-[3.5rem] p-10 border border-stone-200 shadow-xl relative overflow-hidden group">
          ...
          <div className="flex flex-col sm:flex-row gap-5 relative">
            <a href={`tel:${data.contacts.phone}`} className="flex-1 bg-white hover:bg-stone-50 p-6 rounded-[1.8rem] flex items-center justify-center gap-4 transition-all border-2 border-stone-100 hover:border-amber-200 active:scale-95 group/btn shadow-sm">
            ...
            <a href={`https://wa.me/${cleanPhone(data.contacts.whatsapp)}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-amber-800 hover:bg-amber-900 p-6 rounded-[1.8rem] flex items-center justify-center gap-4 transition-all shadow-lg shadow-amber-900/10 active:scale-95 group/wa">
              <MessageCircle className="w-5 h-5 text-white" />
              <span className="font-black text-xs text-white uppercase tracking-widest">WhatsApp</span>
            </a>
          </div>

          {(data.contacts.website || data.contacts.instagram) && (
            <div className="mt-4 flex flex-col sm:flex-row gap-3">
              {data.contacts.website && (
                <a href={`https://${data.contacts.website}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-stone-50 hover:bg-stone-100 p-4 rounded-2xl flex items-center justify-center gap-3 transition-all border border-stone-200 active:scale-95 text-stone-600 hover:text-stone-900">
                  <Globe className="w-4 h-4" />
                  <span className="font-bold text-xs">{data.contacts.website}</span>
                </a>
              )}
              {data.contacts.instagram && (
                <a href={`https://instagram.com/${data.contacts.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-stone-50 hover:bg-stone-100 p-4 rounded-2xl flex items-center justify-center gap-3 transition-all border border-stone-200 active:scale-95 text-stone-600 hover:text-stone-900">
                  <Camera className="w-4 h-4" />
                  <span className="font-bold text-xs">{data.contacts.instagram}</span>
                </a>
              )}
            </div>
          )}
          ...
          <div className="mt-6 grid grid-cols-3 gap-2">
            {[
              ...
              <a key={i} href={`https://wa.me/${cleanPhone(data.contacts.whatsapp)}?text=${encodeURIComponent(msg.label)}`} target="_blank" rel="noopener noreferrer" className="bg-stone-50 border border-stone-200 hover:bg-amber-50 hover:border-amber-200 rounded-xl p-3 flex flex-col items-center justify-center gap-2 text-center transition-all active:scale-95 group">
          ...
          {/* Review Link */}
          <a href="https://g.page/r/YOUR_GOOGLE_REVIEW_LINK" target="_blank" rel="noopener noreferrer" className="mt-6 w-full p-4 rounded-2xl bg-gradient-to-r from-amber-100 to-orange-100 border border-amber-200 flex items-center justify-between group cursor-pointer">
            <div className="flex items-center gap-3">
